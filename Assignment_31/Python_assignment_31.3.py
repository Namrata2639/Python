...
Q.3 Design automation script which accept two directory names. 
...
def CreateDirectory(SrcDict,DestDict):
...
def setup_logging(filename="CreateDirectory.log"):
...
def main():
    ...
    if (len(sys.argv) != 3):
        ...
        logging.info("--usage:command filename.py srcdirecctory destdirectory")
        return
    
    sourcedict = sys.argv[1]
    destdict   = sys.argv[2]

    CreateDirectory(sourcedict,destdict)
