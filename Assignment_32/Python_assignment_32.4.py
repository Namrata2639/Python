...
def find_duplicate(DirName):
    ...
    if not os.path.exists(DirName):
        logging.critical(DirName+ "No such Directory Exists")
        return
    
    if not os.path.isdir(DirName):
        logging.critical(DirName+" is not a directory")
        return
...
def main():
    ...
    logging.info(f"Execution time for the script: {execution_time} seconds")
